...
const tabButtonClass = (activeTab: TabKey, tab: TabKey) =>
  `rounded-hdb px-3 py-2 text-sm font-medium transition-colors ${
    activeTab === tab
      ? "bg-heritage-navy text-white shadow-fintech"
...
export function Navigation({ activeTab, onSelectTab }: NavigationProps) {
  return (
    <nav className="fixed top-0 z-50 flex w-full items-center justify-between border-b border-heritage-navy/10 bg-white/90 px-5 py-4 backdrop-blur md:px-8">
      <div className="flex items-center gap-2">
        <div className="flex h-8 w-8 items-center justify-center rounded-hdb bg-heritage-navy">
          <svg
            aria-hidden="true"
            width="18"
            height="18"
          ...
          >
            <path d="M4 22V10L12 4L20 10V22H14V16H10V22H4Z" fill="white" />
          </svg>
        </div>
        <span className="text-lg font-bold">HDB Planner</span>
      </div>
